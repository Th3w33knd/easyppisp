...
__device__ __forceinline__ void compute_homography_skew_matrix_construction_bwd(
    ...
    grad_t_gray.x = -grad_skew(1, 2) + grad_skew(2, 1);
    grad_t_gray.y = grad_skew(0, 2) - grad_skew(2, 0);
    grad_t_gray.z = -grad_skew(0, 1) + grad_skew(1, 0);
}
...
__device__ __forceinline__ void compute_homography_matrix_T_construction_bwd(const float3x3 &grad_T,
                                                                              ...
                                                                              float3 &grad_t_r,
                                                                              float3 &grad_t_g) {
    grad_t_b = make_float3(grad_T(0, 0), grad_T(1, 0), grad_T(2, 0));
    grad_t_r = make_float3(grad_T(0, 1), grad_T(1, 1), grad_T(2, 1));
    grad_t_g = make_float3(grad_T(0, 2), grad_T(1, 2), grad_T(2, 2));
}
...
__device__ __forceinline__ void compute_homography_nullspace_computation_bwd(
    const float3x3 &M, const float3 &lambda_v, const float3x3 &grad_D_for_nullspace, float3x3 &grad_M) {
    float3 r0 = make_float3(M(0, 0), M(0, 1), M(0, 2));
    float3 r1 = make_float3(M(1, 0), M(1, 1), M(1, 2));
    float3 r2 = make_float3(M(2, 0), M(2, 1), M(2, 2));

    float3 grad_lambda = make_float3(grad_D_for_nullspace(0, 0), grad_D_for_nullspace(1, 1), grad_D_for_nullspace(2, 2));
    float3 grad_r0 = make_float3(0, 0, 0), grad_r1 = make_float3(0, 0, 0), grad_r2 = make_float3(0, 0, 0);

    float3 l_test1 = cross(r0, r1);
    if (dot(l_test1, l_test1) > 1.0e-20f) {
        cross_bwd(grad_lambda, r0, r1, grad_r0, grad_r1);
    } else {
        float3 l_test2 = cross(r0, r2);
        if (dot(l_test2, l_test2) > 1.0e-20f) {
    ...
    grad_M = make_float3x3(grad_r0.x, grad_r0.y, grad_r0.z, grad_r1.x, grad_r1.y, grad_r1.z, grad_r2.x, grad_r2.y, grad_r2.z);
}
...
#pragma unroll
    for(int k=0; k<4; k++) ((float*)&zca_b)[k] = COLOR_PINV_BLOCKS[0][k];
#pragma unroll
    for(int k=0; k<4; k++) ((float*)&zca_r)[k] = COLOR_PINV_BLOCKS[1][k];
#pragma unroll
    for(int k=0; k<4; k++) ((float*)&zca_g)[k] = COLOR_PINV_BLOCKS[2][k];
#pragma unroll
    for(int k=0; k<4; k++) ((float*)&zca_n)[k] = COLOR_PINV_BLOCKS[3][k];
    ...
    float3 t_b = make_float3(bd.x, bd.y, 1.0f);
    float3 t_r = make_float3(1.0f+rd.x, rd.y, 1.0f);
    float3 t_g = make_float3(gd.x, 1.0f+gd.y, 1.0f);
    ...
    float3x3 skew = make_float3x3(0, -t_gray.z, t_gray.y, t_gray.z, 0, -t_gray.x, -t_gray.y, t_gray.x, 0);
    float3x3 M = skew * T;

    float3 lambda_v;
    float3 r0 = make_float3(M(0,0), M(0,1), M(0,2)), r1 = make_float3(M(1,0), M(1,1), M(1,2)), r2 = make_float3(M(2,0), M(2,1), M(2,2));
    lambda_v = cross(r0, r1);
    if(dot(lambda_v, lambda_v) < 1.0e-20f) {
        lambda_v = cross(r0, r2);
        if(dot(lambda_v, lambda_v) < 1.0e-20f) lambda_v = cross(r1, r2);
    }

    float3x3 D = make_float3x3(lambda_v.x, 0, 0, 0, lambda_v.y, 0, 0, 0, lambda_v.z);
    float3x3 S_inv = make_float3x3(-1,-1,1, 1,0,0, 0,1,0);
    float3x3 TD = T * D;
    ...
    float3x3 grad_TD = grad_H_unnorm * transpose(S_inv);
    ...
    float3x3 grad_M;
    compute_homography_nullspace_computation_bwd(M, lambda_v, grad_D, grad_M);

    float3x3 grad_skew, grad_T_from_M;
    mul_mat_bwd(skew, T, grad_M, grad_skew, grad_T_from_M);
#pragma unroll
    for(int i=0; i<9; i++) grad_T.m[i] += grad_T_from_M.m[i];

    float3 grad_t_gray;
    compute_homography_skew_matrix_construction_bwd(grad_skew, grad_t_gray);
    float3 gtb, gtr, gtg;
    compute_homography_matrix_T_construction_bwd(grad_T, gtb, gtr, gtg);

    float2 gbd, grd, ggd, gnd;
    compute_homography_target_point_bwd(gtb, gbd);
    compute_homography_target_point_bwd(gtr, grd);
    compute_homography_target_point_bwd(gtg, ggd);
    compute_homography_target_point_bwd(grad_t_gray, gnd);

    float2x2 dummy;
    float2 gb_lat, gr_lat, gg_lat, gn_lat;
    mul_bwd(zca_b, b_lat, gbd, dummy, gb_lat);
    mul_bwd(zca_r, r_lat, grd, dummy, gr_lat);
    mul_bwd(zca_g, g_lat, ggd, dummy, gg_lat);
    mul_bwd(zca_n, n_lat, gnd, dummy, gn_lat);

    grad_color_params->b += gb_lat;
    grad_color_params->r += gr_lat;
    grad_color_params->g += gg_lat;
    grad_color_params->n += gn_lat;
}
...
__device__ __forceinline__ void apply_color_correction_ppisp_bwd(
    ...
    float3 grad_rgi_out_norm = make_float3(grad_rgb_out.x - grad_rgb_out.z, grad_rgb_out.y - grad_rgb_out.z, grad_rgb_out.z);
    float3 grad_rgi_out = grad_rgi_out_norm * norm_factor;
    float grad_norm = dot(grad_rgi_out_norm, rgi_out);
    grad_rgi_out.z += -grad_norm * norm_factor / (rgi_out.z + 1.0e-5f);

    float3x3 grad_H;
    float3 grad_rgi_in;
    mul_bwd(H, rgi_in, grad_rgi_out, grad_H, grad_rgi_in);

    grad_rgb_in = make_float3(grad_rgi_in.x + grad_rgi_in.z, grad_rgi_in.y + grad_rgi_in.z, grad_rgi_in.z);
    float grad_int = (intensity > 1e-8f) ? grad_norm * norm_factor / intensity : 0.0f;
    grad_rgb_in.x += grad_int; grad_rgb_in.y += grad_int; grad_rgb_in.z += grad_int;
...
__device__ __forceinline__ void apply_crf_ppisp_bwd(const float3 &rgb_in,
                                                    ...
                                                    const float3 &grad_rgb_out, float3 &grad_rgb_in,
                                                    CRFPPISPChannelParams *grad_crf_local) {
    float3 rgb_c = clamp(rgb_in, 0, 1);
    float rgb_a[3] = {rgb_c.x, rgb_c.y, rgb_c.z}, gout[3] = {grad_rgb_out.x, grad_rgb_out.y, grad_rgb_out.z}, gin[3] = {0,0,0};

#pragma unroll
    for(int i=0; i<3; i++) {
        const CRFPPISPChannelParams &p = crf_params[i];
        float toe = bounded_positive_forward(p.toe, 0.3f), shoulder = bounded_positive_forward(p.shoulder, 0.3f), gamma = bounded_positive_forward(p.gamma, 0.1f), center = clamped_forward(p.center);
        float lval = __fmaf_rn(shoulder - toe, center, toe), a = __fdividef(shoulder * center, lval), b = 1.0f - a;
        float x = rgb_a[i], y = (x <= center) ? a * __powf(__fdividef(x, center), toe) : 1.0f - b * __powf(__fdividef(1.0f - x, 1.0f - center), shoulder);
        float output = __powf(fmaxf(0.0f, y), gamma), y_c = fmaxf(0, y);

        float gy = (y_c > 0) ? gout[i] * gamma * __powf(y_c, gamma - 1.0f) : 0.0f;
        float gx = 0;
        if(x <= center && center > 0) {
            float base = x/center;
            if(base > 0) gx = gy * a * toe * __powf(base, toe - 1.0f) / center;
        } else if(x > center && center < 1) {
            float base = (1-x)/(1-center);
            if(base > 0) gx = gy * b * shoulder * __powf(base, shoulder - 1.0f) / (1-center);
        }
        gin[i] = gx;

        float g_toe = 0, g_sh = 0, g_ga = (y_c > 0) ? gout[i] * output * __logf(y_c + 1e-8f) : 0, g_ce = 0;
        float ga = 0, gb = 0;
        if(x <= center && center > 0) {
            float base = x/center;
            if(base > 0) {
                float pw = __powf(base, toe);
                ga += gy * pw;
                g_toe += gy * a * pw * __logf(base + 1e-8f);
                g_ce += gy * a * toe * __powf(base, toe-1) * (-x/(center*center));
            }
        } else if(x > center && center < 1) {
            float base = (1-x)/(1-center);
            if(base > 0) {
                float pw = __powf(base, shoulder);
                gb += -gy * pw;
                g_sh += -gy * b * pw * __logf(base + 1e-8f);
                g_ce += gy * (-b * shoulder * __powf(base, shoulder-1)) * ((1-x)/((1-center)*(1-center)));
            }
        }
        ga += -gb;
        float gl = 0;
        if(fabsf(lval) > 1e-8f) {
            g_sh += ga * center / lval; g_ce += ga * shoulder / lval; gl = -ga * (shoulder*center)/(lval*lval);
        }
        g_sh += gl * center; g_toe += gl * (1-center); g_ce += gl * (shoulder - toe);

        grad_crf_local[i].toe += bounded_positive_backward(p.toe, 0.3f, g_toe);
        grad_crf_local[i].shoulder += bounded_positive_backward(p.shoulder, 0.3f, g_sh);
        grad_crf_local[i].gamma += bounded_positive_backward(p.gamma, 0.1f, g_ga);
        grad_crf_local[i].center += clamped_backward(p.center, g_ce);
    }
    grad_rgb_in = make_float3(gin[0], gin[1], gin[2]);
}
